...
class HaloMassFcn:
    def __init__(self, radii = None, log_mass_scale = 14.0, halo_file=None,omega_matter0=0.27, omega_lambda0=0.73,
    omega_baryon0=0.044, hubble0=0.71, sigma8input=0.801, primordial_index=0.95,
    this_redshift=0.0, log_mass_min=None, log_mass_max=None, num_sigma_bins=360,
    fitting_function=3, mass_column=0, delta=200):
        """
        ...
        *not* stored in the enzo datset so it must be checked by hand.
        ...
        if radii is not None:
            ...
            # mean mass density at z=0
            rho0 = self.omega_matter0 * 2.78e+11; # in units of h^2 Msolar/Mpc^3
 
            # mass in units of h^-1 Msolar
            thismass = math.pow(10.0, log_mass_scale) *self.hubble0

            self.b = self.bias_of_M(self.this_redshift, thismass)
            print self.b
            # radius is in units of h^-1 Mpc (comoving)
            self.smoothing_scale = math.pow( 3.0*thismass / 4.0 / math.pi / rho0, 1.0/3.0 )
            print self.smoothing_scale
            self.velCorr()
        elif self.halo_file is not None:
...
"""
    ...
    def read_haloes(self):
        ...
        """
        print("Reading halo masses from %s" % self.halo_file)
    ...
    def velCorr(self):
        ...
        self.psiperparray = na.zeros(len(self.Rarray))
        self.psipararray = na.zeros(len(self.Rarray))

        bar = pb.ProgressBar(maxval=len(self.Rarray), term_width = 100, widgets=[pb.Bar('=','[',']'), ' ', pb.Percentage()])
        i = 0
        for R in self.Rarray:

            bar.update(i+1)
            self.psiperparray[i] = self.psi_perp_of_R(R)            
            self.psipararray[i] = self.psi_par_of_R(R)
    ...
    def sigmaM(self):
        ...
        # 6) Psi_par(R, where R is in Mpc/h)
        self.xiarray = na.empty(self.num_sigma_bins, dtype='float64')
        ...
        for i in xrange(self.num_sigma_bins):
            ...
            self.sigmaarray[i] = math.sqrt(self.sigma_squared_of_R(R, norm=True))
            self.xiarray[i] = self.xi_of_R(R)
    ...
    def norm_power(self):
        ...
        sigma8_unnorm = math.sqrt(self.sigma_squared_of_R(R, norm=False))

        norm = (self.sigma8input / sigma8_unnorm)**2.0

        return norm
 
    def sigma_squared_of_R(self, R, norm=True):
        """
        ...
        */
        ...
        if (norm):
            return self.norm_power()*sigmasquaredofR
        else:
    ...
    def multiplicityfunction(self, sigma):
        ...
        if self.fitting_function==1:
            ...
            print("Don't understand this.  Fitting function requested is %d\n",
    ...
    def delta_critofz(self, redshift):

        thisdelta = 1.686 / self.Dofz(redshift)

        return thisdelta

    def bias_of_M(self, redshift, M):

        y = na.log10(200.0)
        
        A = 1.0 + 0.24*y*na.exp(-(4./y)**4.)
        a = 0.44*y - 0.88

        B = 0.183
        b = 1.5
        
        C = 0.019 + 0.107*y + 0.19*na.exp(-(4./y)**4.)
        c = 2.4
        
        deltaCritofz = self.delta_critofz(redshift)

        rho0 = self.omega_matter0 * 2.78e+11; # in units of h^2 Msolar/Mpc^3
        thisradius = math.pow( 3.0*M / 4.0 / math.pi / rho0, 1.0/3.0 )
        
        sigmaofz = self.Dofz(redshift) * self.sigma_squared_of_R(thisradius, norm=True)
    ...
    def bias(self, redshift):

        """
        @ calculated using equation 6 in Tinker et al. 2010
        @ overdensity parameter = self.delta (wrt to mean density)
        """
        ...
        for i in range(len(self.massarray)):
            sigmaofz.append(self.sigmaof_M_z(i, redshift))

        sigmaofz = na.array(sigmaofz)
    ...
    def fgrowth(self, z):
        ...
        omegaMofz = self.omega_matter_of_z(z)
        omegaLofz = self.omega_lambda_of_z(z)

        
        return omegaMofz**(4/7.) + (1. + omegaMofz/2.0)*(omegaLofz/70.)

    def xi_of_R(self, R):
        """ calculates the correlation function as function of radius R

        Note that R is in h^-1 Mpc comoving"""

        self.R = R
        result = integrate_inf(self.xi_of_R_integrand)

        xi = result / 2.0 / math.pi / math.pi

        return xi*self.norm_power()

    def xi_of_R_integrand(self, k):

        Rcom = self.R
        x = Rcom*k
        j0 = na.sin(x) / x
        
        f = k*k*self.PofK(k)*j0
    ...
    def v12_of_R(self, R):
        ...
        self.R = R
        result = integrate_inf(self.v12_of_R_integrand)

        factor = -2.0 * self.fgrowth(0.0) / (1.0 + self.xi_of_R(R))
        
        v12 = result / 2.0 / math.pi / math.pi * factor * 100.0*self.hubble0

        return v12*self.norm_power()
        
    def v12_of_R_integrand(self, k):

        Rcom = self.R
        x = Rcom*k
        j1 = na.sin(x) / x / x - na.cos(x) / x

        f = k*self.PofK(k)*j1

        return f

    def sigmaV_squared_of_R(self, R):

        result = integrate_inf(self.sigmaV_squared_integrand)

        factor = (self.fgrowth(0.0)*100.0*self.hubble0)**2/3.0
        sigmasquared = result / 2.0 / math.pi / math.pi * factor

        x = sigmasquared*self.norm_power()
        
        return x
 
    def sigmaV_squared_integrand(self, k):
        """
        /* integrand for integral to get sigma^2(R). */
        """
 
        Rcom = self.smoothing_scale;  # this is R in comoving Mpc/h
 
        f = self.PofK(k)*na.power( abs(self.WofK(Rcom,k)), 2.0)
 
        return f

    
    def psi_perp_of_R(self, R):

        self.R = R
        result = integrate_inf(self.psi_perp_integrand)

        factor = (self.fgrowth(0.0)*100.0*self.hubble0)**2
        psi = result / 2.0 / math.pi / math.pi * factor

        
        return psi*self.norm_power()
 
    def psi_perp_integrand(self, k):
        ...
        f = self.PofK(k)*na.power( abs(self.WofK(Rcom,k)), 2.0) * j1 / x
 
        return f

    def psi_par_of_R(self, R,):

        self.R = R
        result = integrate_inf(self.psi_par_integrand)

        factor = (self.fgrowth(0.0)*100.0*self.hubble0)**2.0
        term1 = result / 2.0 / math.pi / math.pi * factor * self.norm_power()

        term2 = -2.0*self.psi_perp_of_R(R)

        
        term3 = -1./4.0*(self.v_par_mean(R)**2. * (1. + self.b**2 * self.xi_of_R(R)))

        psi  = term1 + term2 + term3

        return psi
 
    def psi_par_integrand(self, k):
        ...
        f = self.PofK(k)*na.power( abs(self.WofK(Rcom,k)), 2.0) * j0
 
        return f

    def v_par_mean(self, R):
        ...
        return (self.b*v12*(1. + xi))/(1. + self.b**2. * xi)
